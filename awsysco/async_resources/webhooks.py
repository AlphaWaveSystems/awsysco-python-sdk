...
class AsyncWebhooksResource:
    ...
    async def list(self) -> dict:
        return await self._http.get("/api/v1/webhooks") or {}
    ...
    async def create(self, url: str, events: List[str], *, name: Optional[str] = None, secret: Optional[str] = None) -> Webhook:
        ...
        if secret is not None:
            body["secret"] = secret
        data = await self._http.post("/api/v1/webhooks", json=body)
        return Webhook.model_validate(data)

    async def update(self, webhook_id: str, **kwargs: Any) -> Webhook:
        # No /api/v1 alias exists for this route on the platform.
        body: Dict[str, Any] = {}
    ...
    async def delete(self, webhook_id: str) -> dict:
        return await self._http.delete(f"/api/v1/webhooks/{webhook_id}") or {}

    async def test(self, webhook_id: str, event_type: str) -> dict:
        return await self._http.post(f"/api/v1/webhooks/{webhook_id}/test", json={"eventType": event_type}) or {}
